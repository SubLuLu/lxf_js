...
var checkRegisterForm = function () {
    // 获取各个节点
    let rootRegElem = document.querySelector('#test-register');
    let errorTipElem = rootRegElem.querySelector('#test-error');
    let userNameElem = rootRegElem.querySelector('#username');
    let passwordElem = rootRegElem.querySelector('#password');
    let checkPwdElem = rootRegElem.querySelector('#password-2');

    // 初始化错误信息列表
    let errorTipArray = ['用户名必须是3-10位英文字母或数字', '口令必须是6-20位', '两次输入口令必须一致'];
    // 初始化错误提示节点
    errorTipElem.innerText = '';

    // 利用正则表达式判断用户名是否正确
    let filter = /^[\w\d]{3,10}$/;
    if (!filter.test(userNameElem.value)){
        errorTipElem.innerText = errorTipArray[0];
        return false;
    }
    // 利用正则表达式判断密码是否正确
    filter = /^[\w\d]{6,20}$/;
    if (!filter.test(passwordElem.value)){
        errorTipElem.innerText = errorTipArray[1];
        return false;
    }
    // 判断两次输入的密码是否相同
    if (passwordElem.value !== checkPwdElem.value){
        errorTipElem.innerText = errorTipArray[2];
        return false;
    }
    return true;
}
